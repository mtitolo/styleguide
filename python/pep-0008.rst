...
The generally preferred format for a statement split across multiple
lines is the diff-friendly format::
...
However a terser form is acceptable if it's not likely the contents
of the statement will vary independently::
...
- If an import gets too long to fit on one line, it should be split
  across lines in the diff-friendly way::
